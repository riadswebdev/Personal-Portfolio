...
export default function SkillsSection() {
  ...
  const skills = [
    { name: "HTML & CSS",    percentage: 95, offset: Math.round(251.2 * (1 - 0.95)) },
    { name: "JavaScript",    percentage: 88, offset: Math.round(251.2 * (1 - 0.88)) },
    { name: "React.js",      percentage: 83, offset: Math.round(251.2 * (1 - 0.83)) },
    { name: "Next.js",       percentage: 80, offset: Math.round(251.2 * (1 - 0.80)) },
    { name: "Tailwind CSS",  percentage: 92, offset: Math.round(251.2 * (1 - 0.92)) },
    { name: "Git & GitHub",  percentage: 85, offset: Math.round(251.2 * (1 - 0.85)) },
  ];
    ...
    <section ref={container} className="py-section-gap" id="skills">
      <div className="text-center mb-stack-lg space-y-4">
        <h2 className="font-h2 text-h2">
          Technical <span className="text-primary-container">Expertise</span>
        </h2>
        ...
        {skills.map((skill, index) => (
          ...
          >
            ...
            <div className="relative w-20 h-20 md:w-24 md:h-24 flex items-center justify-center">
              ...
              <span className="absolute text-sm md:text-lg font-bold">{skill.percentage}%</span>
            </div>
      ...
      <div className="relative w-full overflow-hidden py-4 border-y border-white/5 bg-surface-container-lowest/50">
        ...
        <div className="animate-marquee gap-8 items-center">
          {[...techIcons, ...techIcons].map((tech, i) => (
            <div key={i} className="flex items-center gap-3 glass-card px-6 py-3 rounded-full whitespace-nowrap cursor-pointer hover:scale-105 transition-transform">
              <span className="text-xl">{tech.icon}</span>
              <span className="font-label-caps text-sm text-on-surface-variant group-hover:text-white transition-colors">{tech.name}</span>
            </div>
          ))}
        </div>
      </div>
